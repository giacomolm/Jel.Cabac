			...
			xsdAttr.fn = xsdAttr.prototype = {
				...
				fetchElement: function(xmlDoc, elName, result){
					...
					if(xmlElement){
						...
						//if the xmlQuery referred to attribute tag contains some element, the grab its attribute 
						if(xmlQuery){
						...
						//Recalculating extension element, it can be lost in the previous fetch element
						/*if(xmlElement.getElementsByTagName('extension').length > 0){ //chrome case
							xmlExstension = xmlElement.getElementsByTagName('extension');
						}
						else{
						...
						//Checking if the element has sub elements with complex type 
						if(xmlElement.getElementsByTagName('element').length > 0){ //chrome case
							xmlQuery = xmlElement.getElementsByTagName('element');
						}
						else if(xmlElement.getElementsByTagName('xs:element').length > 0){//firefox,
							xmlQuery = xmlElement.getElementsByTagName('xs:element');
						}
						else if(((window.ActiveXObject+"").substring(0,9))!="undefined"){//IE
							xmlQuery = xsdAttr.fn.selectElement(xmlDoc, "xs:complexType",elName);
							if(!xmlQuery){
						...
						for(i=0; i<xmlQuery.length; i++){
							...
							if(xmlQuery[i].getAttribute("type")){

								if(xmlQuery[i].getAttribute("type").split(":")[0] == "xs"){
								  result[xmlQuery[i].getAttribute("name")] = undefined;
							    }
							    else xsdAttr.fn.fetchElement(xmlDoc,xmlQuery[i].getAttribute("type"), result);
							}
						}
						//end Element Case	
