define("raphaelext", ['raphael'], function(){
	...
	Raphael.fn.shape = function(url, x, y, width, height, context, definition){
		...
		shapeEl.addElement = function(obj, number, labels, is_array){ 
			if(!number) number = 1;
			var i;
			...
			for(i=0; i<Math.min(2,number); i++){
				...
				if(labels){
					...
					if(is_array)
						curr_txt.id = curr_el.id+","+i+",label";
					else curr_txt.id = curr_el.id+",label";
			...
			if(number>2){
				...
				if(labels){
					...
					if(is_array)
						curr_txt.id = curr_el.id+","+(number-1)+",label";
					else curr_txt.id = curr_el.id+",label";
