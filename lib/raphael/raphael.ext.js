		...
		shapeEl.addElement = function(obj, number, labels, is_array, definition){ 
			...
			for(i=0; i<Math.min(2,number); i++){
				...
				if(labels.length==undefined || labels.length>0){
					//building the graphical element that will be added to the canvas
					...
					//adding the element to elements collection
			...
			console.log(this.attrs.width, (this.additionalEl.length*(obj.width+10))+obj.shiftX);
			if(number>2){
